...
$pdo->exec("INSERT INTO goals (id, slug, is_active) VALUES (1,'travel_companion',1), (2,'emotional_connection',1)");
...
oassert(is_bool($stateAfterPrefs['goal_questions'] ?? null), 'goal-questions completion state should be computed');

// Completion guard should use the same ordered primary-goal keys as onboarding render logic
// and ignore extra required DB definitions not currently rendered.
$pdo->exec("INSERT INTO goal_preference_definitions (goal_id, pref_key, label_key, helper_text_key, input_type, value_type, allowed_values_json, weight, is_required, is_active) VALUES
    (2,'legacy.hidden_required_key','x',NULL,'select','string','[\"a\",\"b\"]',1,1,1)");
$stateWithHiddenRequired = $repo->completionState(7);
oassert(($stateWithHiddenRequired['goal_questions'] ?? false) === true, 'hidden required keys outside rendered ordered catalog should not fail completion guard');
